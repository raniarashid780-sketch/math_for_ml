...
# After p_six:
print(f"P(roll = 6) = {p_six:.4f}")

assert np.isclose(p_six, 1/6, atol=0.02), "P(6) too far from 1/6"
...
assert np.isclose(p_even, 0.5, atol=0.02), "P(even) too far from 0.5"


print(f"P(>3) * P(even) = {p_gt3 * p_even:.4f}")
print(f"P(>3 and even)  = {p_both:.4f}")

print(f"Independent: {np.isclose(p_gt3 * p_even, p_both, atol=0.02)}")
